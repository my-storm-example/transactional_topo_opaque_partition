...
public class MyOpaquePtTxSpout implements IOpaquePartitionedTransactionalSpout<MyMata> {
  ...
  public MyOpaquePtTxSpout() {
    ...
    for (int j = 0; j < 5; j++) {
      ...
      for (long i = 0; i < 100; i++) {
        dbMap.put(i,
            hosts[0] + "\t" + session_id[random.nextInt(5)] + "\t" + time[random.nextInt(8)]);
      }
  ...
  public class MyEmitter implements IOpaquePartitionedTransactionalSpout.Emitter<MyMata> {
    ...
    @Override
    public MyMata emitPartitionBatch(TransactionAttempt tx, BatchOutputCollector collector,
        int partition, MyMata lastPartitionMeta) {
      // TODO Auto-generated method stub
